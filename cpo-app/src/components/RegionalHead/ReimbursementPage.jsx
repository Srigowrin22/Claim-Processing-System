...
import {
  Box, Typography, Grid, Card, CardContent, CardActions, Button, Stack, Divider, Chip, TextField, Dialog, DialogTitle, DialogContent, DialogActions, BottomNavigation, BottomNavigationAction, Paper, IconButton, Tooltip
} from '@mui/material';
...
// --- Updated ValidateDialog with Approved Amount input and validation ---
function ValidateDialog({
  ...
  const isApproveDisabled =
    ...
    Number(approvedAmount) > (claim?.expected_amount || 0);
...
export default function ReimbursementPage() {
  ...
  const filteredClaims = claims.filter(c =>
    (c.hospital_id === null || c.hospital_id === undefined) &&
  ...
  const handleValidate = async (claim, status) => {
    ...
    try {
      // Auto-set feedback to status if empty
      const feedbackToSend = feedback.trim() === '' ? status : feedback;
