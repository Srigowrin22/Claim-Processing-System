...
export default function Reimbursement() {
  ...
  // Render card (for both search and lists)
        ...
        <CardActions sx={{ mt: 'auto' }}>
          <Stack direction="row" spacing={1} sx={{ width: '100%', justifyContent: 'center' }}>
            ...
            <Button
              ...
              disabled={!c.canAct || c.status === 'approved' || c.status === 'rejected'}
            >
              Approve
            </Button>
            <Button
  ...
  // Tab content (all tabs use the same grid layout)
  let tabClaims = [];
  let showActions = false;
  if (tab === 'recent') {
    tabClaims = recentClaims;
    showActions = false;
  } else if (tab === 'pending') {
    tabClaims = pendingClaims;
    showActions = true;
  } else if (tab === 'completed') {
    tabClaims = completedClaims;
    showActions = false;
  }
  ...
  return (
    <Box sx={{ p: 4, maxWidth: 900, mx: 'auto', pb: 8 }}>
      ...
      {/* Searched Claim */}
      {claim && (
        <Grid container spacing={2} alignItems="stretch" sx={{ mb: 2, width: '100%' }}>
